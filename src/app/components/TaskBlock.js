...
class TaskBlock extends React.Component {
    ...
    async handleChange(ev) {
        await axios.put(`/api/tasks/${this.props.task.id}`, { complete: ev.target.checked });
    ...
    async handleSooner(ev) {
        ...
        await axios.put(`/api/tasks/${this.props.task.id}`, { date: moment(date).subtract(1, 'days').toDate() });
        const updatedTasks = (await axios.get(`/api/tasks/${this.props.date.getFullYear()}/${this.props.date.getMonth()}`)).data;
        this.props.setTasks(updatedTasks);
    }
    ...
    async handleLater(ev) {
        ev.preventDefault();
        const { date } = this.props.task;
        await axios.put(`/api/tasks/${this.props.task.id}`, { date: moment(date).add(1, 'days').toDate() });
        const updatedTasks = (await axios.get(`/api/tasks/${this.props.date.getFullYear()}/${this.props.date.getMonth()}`)).data;
        this.props.setTasks(updatedTasks);
    }
